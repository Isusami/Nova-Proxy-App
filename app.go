...
type App struct {
	...
	core              *coreClient
	gsaManager        *proxy.GASManager

	// CPU usage tracking (platform-specific implementation)
	cpuMu        sync.Mutex
	cpuPlatState cpuPlatformState
...
func NewApp() *App {
	...
	a.gsaManager = proxy.NewGASManager(filepath.Join(execDir, "data"))
...
func (a *App) GetGSAConfig() proxy.GASConfig {
	return a.gsaManager.GetConfig()
}

func (a *App) UpdateGSAConfig(cfg proxy.GASConfig) error {
...
func (a *App) setGSADialAddr(addr string) {
	...
	a.proxyServer.SetGASDialAddr(addr)
	if a.core != nil && a.core.Ping() {
		...
		if err := a.core.SetGASDialAddr(addr); err != nil {
...
func (a *App) GetGSAStatus() proxy.GASConfig {
...
func (a *App) TestGSARelay(cfg proxy.GASConfig) proxy.GASTestResult {
